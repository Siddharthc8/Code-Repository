...
def best(prices):
    ...
    for i in range(1, len(prices)):
        ...
        if prices[i] >= prices[i - 1] and prices[i] > min_price:
            #min_price = min(min_price, prices[i])       This statement is not needed cuz if curr > prev then the min value is unchanged right?
