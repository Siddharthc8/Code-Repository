    ...
    while True:
        try:
            ...
            if not directory.strip():
                print("Empty directory chosen. ---> Proceeding with default dir.")
                default_dir = r"D:\VLSI and Microelectronics\System Verilog Assertions\System Verilog Assertions.srcs\sim_1\new"
                directory = front_slash(default_dir)
                # continue
...
def get_valid_files():
    while True:
        try:
            ...
            if file_name.upper() in ["EXIT", "N", "NO", "END", "E"]:
                print("Session Terminated")
                break
            if file_name.upper() in ["CHDIR", "NEW DIR", "NEWDIR"]:
                get_valid_dir()
                get_valid_files()
                break
            ...
            with open(file_name, "w") as f:
                ...
                if "sim_1" in os.getcwd():
                    f.write(f"module {file}();\n\n\nendmodule")
                elif "sources_1" in os.getcwd():
                    f.write(f"module {file}(\n\n\n);\n\n\n\nendmodule")
